...
volume="$(amixer -D pulse sget Master | awk 'END{print $(NF-1),$NF}')" #not working volume%
...
bright="$(cat /sys/class/backlight/intel_backlight/brightness)" #broken
...
mem=""
#"$(df /dev/sda5 -h | awk 'END{printf "[%s] %s/%s",$5,$3,$2}')"  
